...
- `GET /trip_updates/{transit_system}` — SSE stream of live position events (`src/main.py`'s `transit_feed()`). Each event is one trip's *current* stop only - see `/trip_detail` below for the rest of a trip's stops.
  ...
  Before the first real poll, a newly-connected client is immediately sent this system's cached `RecentEventsCache` (`src/services/recent_events_cache.py`) - its 50 most-recently-updated trips, oldest first - so there's something to show right away instead of waiting out this system's poll/rate-limit cycle (up to `min_poll_interval_seconds`, e.g. SF-MTA's 240s). Ranked by GTFS-RT `trip_update.timestamp` (falling back to the feed header's timestamp when a source doesn't set the per-trip one); a trip isn't explicitly evicted when it stops appearing live, it just ages out once 50 fresher ones have taken its place. Empty on a fresh process (nothing cached yet) - same as today's behavior.
...
A system's full config lives on its own `TransitSystem` row - `realtime_url`, `alerts_url` (GTFS-RT ServiceAlerts feed - see `GET /service_alerts` above; optional, most systems don't have one on file yet), `schedule_url`, `default_schedule_url` (the `Route.url` fallback for a source that doesn't publish a URL per route), `auth_required` (schema-only for now - see below), `min_poll_interval_seconds`/`quota_group` (see "Rate limiting a source" below), and `active`. There's no code-level list of systems anymore (no `GTFS_URLS`/`GTFS_METADATA`/`DEFAULT_SCHEDULE_URL_BY_SYSTEM`, which used to live in `src/constants.py`): `src/services/transit_system_detail.py`'s `get_transit_system_config`/`get_active_transit_systems` are the only reads, and every consumer (`main.py`'s endpoints, `src/tasks.py`'s periodic fetches, `fetcher.py`'s CLI) goes through them.
...
**A migration inserting/updating a URL that embeds an API key must never write that key as a literal string in the file.** An earlier 511.org key was hardcoded directly in two migrations and flagged by GitGuardian once this repo went public - since rotated. Read it live instead: `from src.settings import get_settings` and `get_settings().api_key_511_org` (an `Optional[str]`, sourced from a real environment variable - `API_KEY_511_ORG` - never from any committed `.env.*` file, same pattern as `POSTHOG_PROJECT_TOKEN`). In prod it's injected via Secrets Manager (`deployment/main.tf`'s `api_key_511org` data source, hand-provisioned there like the TLS cert/key); locally, export it in your own shell before running a migration or the fetcher against a 511.org-backed system.
...
`min_poll_interval_seconds` alone only throttles one system's own feed in isolation - it has no way to know about a quota actually shared across *several different* `TransitSystem` rows. Confirmed live: every 511.org-backed system draws on the same one 60-requests/hour budget tied to our API key, not one each - its own `RateLimit-Limit`/`RateLimit-Remaining` response headers drop identically no matter which `agency` param or endpoint (TripUpdates vs ServiceAlerts) is hit with that key. `TransitSystem.quota_group` (default `NULL` - not coordinated with anyone) names a shared budget: every row with the same `quota_group` value draws from one rolling-hour counter (`src/services/shared_feed_quota.py`'s `SharedFeedQuota`, budget sizes in `QUOTA_GROUP_LIMITS` - deliberately set a bit below the real limit for headroom). Once a group's budget is spent for the hour, a fetch that's otherwise due (per that system's own `min_poll_interval_seconds`) is skipped and the existing cached feed is served instead, however stale - except a system's very first-ever request, which has no stale fallback to serve and proceeds regardless of group budget.

#### Proactive scheduling for a quota_group (`src/services/quota_group_scheduler.py`)

Relying purely on inbound requests to trigger a quota-aware fetch has a real failure mode under concurrent multi-system load: several systems each wanting a fresh fetch every ~30s burn through a shared 55/hour budget in minutes (confirmed by hand: 8 systems at a 30s interval exhausts it in about 3 minutes), then serve increasingly stale data for the rest of the hour - a sharp cliff, not a graceful slowdown.

`QuotaGroupScheduler` (one instance per active `quota_group`, started as a background task from `main.py`'s `lifespan()`) fixes this by proactively spending the group's budget on an even, predictable cadence instead: one real fetch every `3600 / budget` seconds, round-robining which member gets it, so every system gets an equal turn (e.g. roughly every ~11 minutes for an 8-member group at a 55/hour budget) rather than a burst-then-cliff pattern. Every 5th tick services `ServiceAlerts` instead of `TripUpdates` (an ~80/20 split, since alerts don't need anywhere near live-position freshness). `RealtimeFeedCache`'s own demand-driven `quota_group` path (above) is unchanged and still serves as a recovery/bootstrap mechanism - a system's very first request still bypasses quota with nothing cached to fall back on, and if the scheduler ever falls behind a system's own `min_poll_interval_seconds` (set well above the scheduler's normal per-system visit cadence for exactly this reason), a real HTTP request can still trigger a recovery fetch.

`GET /transit_systems/{transit_system}` exposes each scheduled system's `next_realtime_update_at` (a unix timestamp) - `null` for a system not in any scheduled `quota_group`, meant to back a countdown-to-next-update UI.
